...
{    
     ...
     public function newuser()
    {
        ...
        if (!$myvalues) {
            ...
            $message = "Please activate the account ".anchor('user/activate/'.$data['u_link'],
            	'Activate Now','');
            ...
            if (count($checkAlreadyUser) > 0) {
                ...
                if ($myNewUser) {
                    ...
                    $email->setFrom('peninahkimani0727@gmail.com', 'Activate the account');
                    $email->setTo($data['u_email']);
                    $email->setSubject('Activate the account');
                    $email->setMessage($message);
                    $email->send();
                    $email->printDebugger(['headers']);
                    $session->setFlashdata('message','Check your email for account activation');
                    return redirect()->to(site_url('user/register'));
                }
                else{
        ...
        if (!$myvalues) {
            ...
            if (count($allUsers) > 0) {
                if ($data['u_password'] == $allUsers[0]['u_password']) {
                    ...
                    $sessionData['u_date'] = $allUsers[0]['u_createAt'];
                    $sessionData['u_updated'] = $allUsers[0]['u_updated'];
                    $sessionData['u_status'] = $allUsers[0]['u_status'];
                    $session->set('isLoggedIn',$sessionData);
                    if ($session->get('u_id')) {
                       return  redirect()->to(site_url('leave'));
                    }
                    else{
